...
const FullScreenPlayer: React.FC<FullScreenPlayerProps> = ({
  ...
  return (
    <div 
      className={`fixed inset-0 z-[100] bg-[#1c1c1e] text-white overflow-hidden flex flex-col ${isClosing ? 'animate-slide-down' : 'animate-slide-up'}`}
      onAnimationEnd={onAnimationEnd}
    >
      <div 
        className="absolute inset-0 z-0 opacity-40 scale-110 blur-[100px] transition-all duration-[2s]"
        style={{ 
      ...
      <div className="relative z-10 flex items-center justify-between px-8 py-12">
        <button onClick={handleClose} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 transition-colors">
      ...
      <div className="relative z-10 flex-1 flex flex-col md:flex-row items-center justify-center gap-12 px-8 max-w-6xl mx-auto w-full">
        ...
        <div className="w-full max-w-md flex flex-col justify-center gap-8">
          ...
          <div className="group">
            ...
            >
              <div 
                className="absolute h-full bg-white/80 rounded-full transition-all duration-100" 
          ...
          <div className="flex items-center justify-between">
            <button 
              ...
              className={`transition-colors ${isShuffled ? '' : 'text-white/40 hover:text-white'}`}
            ...
            <div className="flex items-center gap-8">
              <button onClick={onPrev} className="text-white hover:text-white/70 transition-colors active:scale-90 transform duration-150">
                <SkipBack className="w-10 h-10 fill-current" />
              </button>
              <button 
                onClick={onPlayPause}
                className="w-20 h-20 rounded-full bg-white text-black flex items-center justify-center hover:scale-105 active:scale-95 transition-all shadow-lg"
              >
                {isPlaying ? <Pause className="w-10 h-10 fill-current" /> : <Play className="w-10 h-10 fill-current ml-1" />}
              </button>
              <button onClick={onNext} className="text-white hover:text-white/70 transition-colors active:scale-90 transform duration-150">
       ...
       <div className="absolute bottom-12 right-12 z-20">
          <button 
            ...
            className={`p-4 rounded-full bg-white/10 backdrop-blur-md transition-all hover:bg-white/20 hover:scale-110 active:scale-95 ${track.isLiked ? '' : 'text-white/40'}`}
