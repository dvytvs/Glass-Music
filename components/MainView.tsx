...
import { Play, MoreHorizontal, Music, Disc, Mic2, Edit, Trash2, ArrowRight, X, Check, Upload, ArrowLeft, Heart, Image } from './Icons';
...
const MainView: React.FC<MainViewProps> = ({ 
  ...
  const renderContent = () => {
    ...
    if (currentView === 'artist_detail' && selectedArtist) {
        // Filter: Check if the track's artist field INCLUDES the selected artist
        // This handles "Marian, Sean" showing up in "Marian" and "Sean" views.
    ...
    if (tracks.length === 0 && currentView !== 'favorites') {
      return (
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 p-2 stagger-list">
          {albums.map(albumName => {
            ...
            return (
              <div 
                key={albumName} 
                className="group flex flex-col gap-3 p-4 rounded-2xl hover:bg-white/5 transition-all cursor-pointer hover:scale-[1.02] active:scale-[0.98]"
                ...
                <div className="aspect-square rounded-xl overflow-hidden shadow-lg relative">
                  ...
                  <div className="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <button 
                      ...
                      className="w-12 h-12 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center text-white hover:scale-110 transition-all"
    ...
    if (currentView === 'artists') {
      // Logic to split artists and get unique individual artists
        ...
        <div className="space-y-2 p-2 stagger-list">
          {artists.map(artist => {
            ...
            return (
                <div 
                    key={artist} 
                    className="flex items-center gap-4 p-4 rounded-xl hover:bg-white/5 transition-colors cursor-pointer group hover:scale-[1.01] active:scale-[0.99]"
                    onClick={() => onGoToArtist(artist)}
                >
                <div className="w-16 h-16 rounded-full bg-white/10 flex items-center justify-center text-white/50 overflow-hidden relative">
                    {artistMeta?.avatar ? (
    ...
    return (
      <div className="space-y-1">
        {(currentView === 'artist_detail' || currentView === 'album_detail' || currentView === 'favorites') && (
            <div className={`flex flex-col mb-8 animate-fade-in-view relative overflow-hidden rounded-3xl ${showHeaderImage ? 'min-h-[300px] justify-end p-8' : 'flex-row items-center gap-6 px-2'}`}>
                     ...
                     <>
                        ...
                        <div className="absolute top-4 right-4 opacity-0 hover:opacity-100 transition-opacity z-20">
                            <button 
                                ...
                                className="bg-black/50 hover:bg-black/70 text-white px-3 py-1.5 rounded-lg text-xs backdrop-blur-md flex items-center gap-2"
                            >
                 ...
                 <div className={`relative z-10 flex items-end gap-6 ${showHeaderImage ? 'w-full' : ''}`}>
                     ...
                     {currentView === 'favorites' && (
                         <div className="w-24 h-24 rounded-xl bg-gradient-to-br from-pink-500 to-rose-600 flex items-center justify-center shadow-lg">
                     ...
                     <div className="flex-1">
                        ...
                        <div className="flex gap-3 mt-6">
                             <button 
                                ...
                                className="text-white px-8 py-3 rounded-full text-base font-bold transition-all shadow-lg flex items-center gap-2 hover:opacity-90 hover:scale-105 active:scale-95"
        ...
        {/* Tracks List with Stagger Animation */}
          ...
          {displayTracks.map((track, index) => {
            ...
            return (
              <div 
                ...
                className={`group grid grid-cols-[auto_1fr_auto] md:grid-cols-[40px_3fr_2fr_1.5fr_120px] gap-4 items-center py-2 px-2 rounded-lg cursor-pointer transition-all duration-200 ${
                  active 
              ...
              >
                ...
                <div className="flex items-center gap-3 min-w-0" onClick={() => onPlay(track)}>
                  <img src={track.coverUrl} alt="" className="w-10 h-10 rounded shadow-md object-cover bg-neutral-800 flex-shrink-0" />
                ...
                <div className="flex items-center justify-end gap-4 relative md:justify-self-end w-full pr-2">
                  ...
                  {isMenuOpen && (
                      <div 
                        ref={menuRef} 
                        onMouseDown={(e) => e.stopPropagation()} 
                        className="absolute top-8 right-0 w-48 bg-[#1e1e20] border border-white/10 rounded-xl shadow-2xl z-50 overflow-hidden animate-zoom-in origin-top-right"
                      >
                          <div className="p-1">
                              <button 
  ...
  return (
    <div className="flex-1 h-full overflow-y-auto pb-32 pt-8 px-8 custom-scrollbar relative">
      ...
      {editingTrack && (
        <div 
          className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-fade-in-view"
          onMouseDown={(e) => e.stopPropagation()}
        >
             <div className="bg-[#1c1c1e] border border-white/10 w-full max-w-lg rounded-2xl shadow-2xl overflow-hidden animate-zoom-in" onMouseDown={e => e.stopPropagation()}>
                    ...
                    <div className="flex gap-6">
                        ...
                        <div className="flex-1 space-y-4">
                            <div>
                                ...
                                <input type="text" value={editingTrack.title} onChange={e => setEditingTrack({...editingTrack, title: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none focus:border-[var(--accent)]" style={{ borderColor: 'rgba(255,255,255,0.1)' }} />
                            </div>
                             <div>
                                <label className="block text-xs font-medium text-white/40 mb-1">Исполнитель</label>
                                <input type="text" value={editingTrack.artist} onChange={e => setEditingTrack({...editingTrack, artist: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none" />
                                <p className="text-xs text-white/20 mt-1">Разделяйте нескольких исполнителей запятой</p>
                            </div>
                             <div className="grid grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-xs font-medium text-white/40 mb-1">Альбом</label>
                                    <input type="text" value={editingTrack.album} onChange={e => setEditingTrack({...editingTrack, album: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none" />
                                </div>
                                <div>
                                    <label className="block text-xs font-medium text-white/40 mb-1">Год</label>
                                    <input type="text" value={editingTrack.year || ''} onChange={e => setEditingTrack({...editingTrack, year: e.target.value})} className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2 text-white text-sm focus:outline-none" placeholder="2024" />
                                </div>
                             </div>
                        </div>
                    </div>
                     <div className="flex justify-end gap-3 pt-2">
                         ...
                         <button type="submit" className="px-6 py-2 rounded-lg text-white text-sm font-medium flex items-center gap-2" style={{ backgroundColor: accentColor }}>
      ...
      {(currentView !== 'artist_detail' && currentView !== 'album_detail' && currentView !== 'favorites') && (
        <div className="flex items-end justify-between mb-8 sticky top-0 z-10 py-4 -mx-8 px-8 backdrop-blur-md bg-transparent transition-all animate-fade-in-view">
            ...
            <button onClick={onShuffleAll} className="bg-white/10 text-white px-6 py-2 rounded-full text-sm font-semibold hover:bg-white/20 hover:scale-105 active:scale-95 transition-all backdrop-blur-md">Перемешать</button>
            </div>
        </div>
      )}

      {(currentView === 'artist_detail' || currentView === 'album_detail' || currentView === 'favorites') && (
          <button onClick={onBack} className="flex items-center gap-2 text-white/50 hover:text-white mb-4 transition-colors animate-fade-in-view absolute top-8 left-8 z-30">
