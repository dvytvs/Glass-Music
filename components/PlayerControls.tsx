...
const PlayerControls: React.FC<PlayerControlsProps> = ({
  ...
  // Helper to split artists for the UI
      ...
      <div className="flex items-center justify-between h-full max-w-screen-2xl mx-auto w-full gap-4">
        ...
        <div className="flex items-center gap-4 w-1/3 min-w-[200px] z-20">
          {currentTrack ? (
            <>
              <div 
                className="w-12 h-12 rounded-lg overflow-hidden shadow-lg relative group cursor-pointer flex-shrink-0"
              ...
              {/* Heart Button - Made always visible */}
              <button 
                onClick={() => onToggleLike(currentTrack.id)}
                className={`transition-colors ml-2 hover:scale-110 active:scale-95 p-1 ${currentTrack.isLiked ? '' : 'text-white/30 hover:text-white'}`}
        ...
        <div className="flex items-center justify-end gap-4 w-1/3 z-20">
           <button onClick={onToggleSidebar} className="text-white/40 hover:text-white transition-colors">
            ...
            <div className="flex items-center gap-2 w-28 group">
             ...
             <button className="text-white/40 transition-colors hidden sm:block hover:text-white" title="AirPlay Unavailable in Browser">
