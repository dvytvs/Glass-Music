...
import { X, Image, Video, Droplet, Upload, Check } from './Icons';
...
const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, config, onUpdate }) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      onUpdate({ backgroundType: type, backgroundSource: url });
    }
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 backdrop-blur-md animate-fade-in-view">
      <div 
        className="w-full max-w-2xl bg-[#1c1c1e] border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-zoom-in"
      ...
      >
            ...
            <div className="flex items-center gap-4">
              <div className="relative group">
                <input 
                  ...
                  className="w-16 h-16 rounded-full overflow-hidden cursor-pointer border-none p-0 bg-transparent transition-transform hover:scale-105 active:scale-95"
                />
          ...
          <section>
            ...
            <div className="grid grid-cols-3 gap-4">
              ...
              <button 
                ...
                className={`h-32 rounded-xl border-2 flex flex-col items-center justify-center gap-2 transition-all hover:scale-[1.02] active:scale-[0.98] ${
              ...
              <button 
                 ...
                 className={`h-32 rounded-xl border-2 border-dashed border-white/20 flex flex-col items-center justify-center gap-2 transition-all hover:scale-[1.02] active:scale-[0.98] hover:border-white/40 hover:bg-white/5 relative overflow-hidden ${
              ...
              >
                {config.backgroundType !== 'liquid' && config.backgroundSource ? (
                    <>
                        ...
                        <div className="relative z-10 bg-black/50 px-3 py-1 rounded-full flex items-center gap-2">
          ...
          <section className="space-y-6">
             <div>
                ...
                <input 
                    ...
                    className="w-full h-2 bg-white/10 rounded-full appearance-none cursor-pointer [&::-webkit-slider-thumb]:appearance-none [&::-webkit-slider-thumb]:w-4 [&::-webkit-slider-thumb]:h-4 [&::-webkit-slider-thumb]:bg-white [&::-webkit-slider-thumb]:rounded-full hover:[&::-webkit-slider-thumb]:scale-110 transition-all"
                />
             </div>
        ...
        <div className="p-6 border-t border-white/10 bg-[#151516] flex justify-end">
            <button 
                onClick={onClose}
                className="px-8 py-3 rounded-xl font-semibold text-white shadow-lg hover:shadow-xl hover:scale-105 active:scale-95 transition-all"
