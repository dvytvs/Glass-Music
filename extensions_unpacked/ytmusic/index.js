...
function normalizeUrl(u) {
  ...
  try {
    ...
    if (Array.isArray(CONFIG.allowlistHosts) && CONFIG.allowlistHosts.length > 0) {
      if (CONFIG.allowlistHosts.indexOf(parsed.hostname) === -1 && !/^https?:\/\//i.test(s)) {
        return null;
      }
    }
...
function isAbsoluteHttpUrl(u) { return isString(u) && /^https?:\/\//i.test(u); } 
...
function extractYt1dConfig(html) {
  ...
  return {
    ajaxURL: ajaxMatch ? ajaxMatch[1].replace(/\\\//g, "/") : "",
...
function extractYt1dDownloadURL(payload) {
  if (!payload) return "";
  if (payload.downloadUrl && /^https?:\/\//i.test(payload.downloadUrl)) return payload.downloadUrl;
  if (payload.downloadURL && /^https?:\/\//i.test(payload.downloadURL)) return payload.downloadURL;
  if (payload.url && /^https?:\/\//i.test(payload.url)) return payload.url;
  if (payload.download_link && /^https?:\/\//i.test(payload.download_link)) return payload.download_link;
...
function extractCobaltDownloadURL(payload) {
  if (!payload) return "";
  if (payload.url && /^https?:\/\//i.test(payload.url)) return payload.url;
  if (payload.audio && /^https?:\/\//i.test(payload.audio)) return payload.audio;
  if (payload.audioUrl && /^https?:\/\//i.test(payload.audioUrl)) return payload.audioUrl;
  if (payload.downloadUrl && /^https?:\/\//i.test(payload.downloadUrl)) return payload.downloadUrl;
  if (payload.downloadURL && /^https?:\/\//i.test(payload.downloadURL)) return payload.downloadURL;
...
function sanitizeTrackBeforeReturn(t) {
  ...
  var sanitized = {
    ...
    duration_ms: (Number(t.duration || 0) || 0) * 1000,  
    cover_url: thumb,                          
